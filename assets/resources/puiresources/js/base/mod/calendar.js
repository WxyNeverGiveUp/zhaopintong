/*-----------------------------------------------------------------------------
* @Description:     新版时间选择组件
* @Version:         2.0.1
* @author:          simon(406400939@qq.com)
* @date             2013-10-29
* ==NOTES:=============================================
* v2.0.0(2013-10-29):
*     仍然采用kissy的calendar组件，其功能非常健全，我只是对当前的api做了一些调整
*     以方便我们更加便捷地使用它
*     refer:
*     http://docs.kissyui.com/docs/html/api/component/calendar
*     todo: 查找calendar函数，判断是否可以实现selectchange
* v2.0.1(2013-11-12):
*     添加refresh函数
*     修复hide bug
* v2.0.2(2014/01/01 14:44:08):
*     添加disabled配置属性
* ---------------------------------------------------------------------------*/

KISSY.add('mod/calendar', function(S, Calendar){
    PW.namespace('mod.Calendar');
    PW.mod.Calendar = Calendar;
},{
    requires:[
        'cal/core'
    ]
...
KISSY.add('cal/core', function(S, Client){
    var
        DOM = S.DOM, get = DOM.get, query = DOM.query,
        STAMP_ATTR = 'data-calendar-id';
    ...
    var 
        Calendar = {
            ...
            client: function(param){
                var 
                    that = this,
                    trigger = get(param.renderTo),
                    newCal;
                if(!!trigger){
                    if(that._hasBuild(trigger)){
                        S.log('此节点下已经有日历了，请检查', 'warn', 'PW/mod/calendar');
                        return null;
                    }
                    newCal = new Client(trigger, param, that._nextCalId++);
                    that.instances.push(newCal);
                }
                return newCal;
            },
            //判断节点上是否有日历
            _hasBuild: function(trigger){
                var
                    that = this,
                    stamp = DOM.attr(trigger, STAMP_ATTR);
                return stamp && stamp != '';
            }
        }
...
KISSY.add('cal/client', function(S, Calendar){
    var
        DOM = S.DOM, get = DOM.get, query = DOM.query, $ = S.all, fn = function(){},
        CONFIG = {
            select:{
                rangeSelect: false,
                minDate: null,
            ...
            style:{
                themeUri: PW.libUrl + 'js/base/assets/calendar/css/style.css',
                pages: 1,
                popup: true,
                startWeek: 0,
                startDay: new Date(S.now()),
                closable: true
            },
            events: {
                //如果返回false, 则默认不写入日期到date
                select: fn,
                rangeSelect: fn,
                monthChange: fn,
                //如果返回false, 则默认写入时间到input
                timeSelect: fn
            }
        },
        //日历设置
        CAL_SETTTINGS = PW.modSettings.CAL_SETTTINGS || {},
        //日历独有样式
        CALENDAR_CLASS = "ks-select-calendar",
        //日历id属性
        STAMP_ATTR = 'data-calendar-id';

    var
        Client = function(trigger, param, calId){
            this._ksCal;
            this._opts;
            this._calId = calId;
    ...
    S.augment(Client, S.EventTarget, {
        _init: function(param){
            var
                that = this;
            that._stamp();
            that._buildParam(param);
            that._loadStyle();
            that.render();
            that._addEvtDispathcer();
        },
        //设置事件
        _addEvtDispathcer: function(){
            var
                that = this;
            //如果区域选择完成
            that._ksCal.on('rangeSelect', that._rangeSelectHandler,that);
            //如果单日期选择完成
            that._ksCal.on('select', that._selectHandler, that);
            //如果时间选择完成
            that._ksCal.on('timeSelect', that._timeSelectHandler, that);
            //月份改变
            that._ksCal.on('monthChange', that._monthChangeHandler, that);
        },
        //选择单个日期的处理事件
        _selectHandler: function(ev){
            var
                that    = this,
                opts    = that._opts,
                trigger = that._trigger,
                date    = ev.date;
            
            //如果回调函数返回true， 则不执行
            if(
                S.isFunction(opts.events.select) && 
                opts.events.select.call(this,trigger,date) == false //notice: 注意回调函数
            ){
                S.log('回调阻止继续写入');
                return;
            }else if(!opts.select.rangeSelect){
                that._writeDate(date);
            }
            that.fire('select', {date: date});
        },
        //日历选择区间后的处理事件
        _rangeSelectHandler: function(ev){
            var
                that    = this,
                opts    = that._opts,
                trigger = that._trigger,
            ...
            //如果回调函数返回true， 则不执行
            if(
                S.isFunction(opts.events.rangeSelect) &&
                opts.events.rangeSelect.call(this, trigger, start, end) == false
            ){
                return;
            }
            else if(opts.select.rangeSelect){
                that._writeDate(start, end);
            }
            that.fire('rangeSelect', {
                start: start, 
                end: end
            });
        },
        _timeSelectHandler: function(ev){
            var
                that    = this,
                opts    = that._opts,
                trigger = that._trigger,
            ...
            //如果回调函数返回true， 则不执行
            if(
                S.isFunction(opts.events.timeSelect) && 
                opts.events.timeSelect.call(this,trigger,date) == false //notice: 注意回调函数
            ){
                S.log('回调阻止继续写入');
                return;
            }else{
            ...
            that.fire('timeSelect', {date: date});
        },
        //月份改变
        _monthChangeHandler: function(ev){
            var
                that    = this,
                opts    = that._opts,
                trigger = that._trigger,
                date    = ev.date,
                month   = date.getMonth();
            if(S.isFunction(opts.events.monthChange)){
                opts.events.monthChange.call(trigger, month)
            }
            that.fire('monthChange', {date: date, month: month});
        },
        //合并参数各级参数
        _buildParam: function(param){
            var
                that = this,
                opts = {};
            //合并参数
            S.mix(opts, CONFIG, true, [], true);
            S.mix(opts, CAL_SETTTINGS, true, [], true);
            S.mix(opts, param, true, [], true);

            //对默认时间格式做一个特殊处理
            if(!opts.select.dateFmt){
                if(opts.select.showTime){
                    opts.select.dateFmt = 'YYYY-MM-DD HH:mm:ss';
                }else{
                    opts.select.dateFmt = 'YYYY-MM-DD';
                }
            }
            that._opts = opts;
        },
        //解析设定参数为ks calendar所需要的参数
        _parseParam: function(opts){
            var
                that = this,
                ret  = {};
            //定义转换参数
            ret.date        = that.fmtDate(opts.style.startDay);
            ret.selected    = that.fmtDate(opts.select.selected);
            ret.startDay    = opts.style.startWeek;
            ret.pages       = opts.style.pages;
            ret.closable    = opts.style.closable;
            ret.rangeSelect = opts.select.rangeSelect;
            ret.minDate     = that.fmtDate(opts.select.minDate);
            ret.maxDate     = that.fmtDate(opts.select.maxDate);
            ret.popup       = opts.style.popup;
            ret.showTime    = opts.select.showTime;
            ret.disabled    = opts.select.disabled;
            return ret;
        },
        _writeDate: function(start, end){
            var
                that    = this,
                trigger = that._trigger,
                opts    = that._opts,
                fmt     = opts.select.dateFmt,
                s, e;
            s = (S.isDate(start)) ? S.moment(start) : null;
            e = (S.isDate(end)) ? S.moment(end) : null;
            if(!s){
                S.log('传入日期数据有误', 'warn', 'pw/mod/calendar');
            }
            //选择的单个日期
            if(s && !e){
                DOM.val(trigger, s.format(fmt));
            }
            //选择的是一个时间区域
            if(s && e){
                DOM.val(trigger, s.format(fmt) + '~' + e.format(fmt));
            }
        },
        //根据解析好的ks calendar参数设置
        _render: function(config){
            var
                that    = this,
                trigger = that._trigger;

            that._ksCal = new Calendar(trigger, config);
        },
        //根据config重新设置calendar，原先的内容销毁
        render: function(config){
            var
                that = this,
                opts = that._opts;
            if(!config){
                config = that._parseParam(opts);
            }
            that._render(config);
        },
        ...
        select: function(date){
            ...
            that.render(that._parseParam(cfg));
            that.show();
        },
        //打开calendar
        show: function(){
        ...
        destroy: function(){
            var
                ...
                trigger = that._trigger;
            that.hide();
            DOM.removeClass(trigger, CALENDAR_CLASS);
            DOM.removeAttr(trigger, STAMP_ATTR);
            that._ksCal.destroy();
        },
        /**
         * 利用moment组建进行时间格式化
         ...
         * @return {Date} 日期格式对象
         */
        fmtDate: function(time, fmt){
            var
                that = this,
                opts = that._opts,
                d    = null;
            if(!time) return d;
            if(S.isDate(time)) return time;
            try{
                if(!fmt) fmt = opts.select.dateFmt;
                if(time){
                    d = S.moment(time, fmt);
                }
            }catch(err){
        ...
        _stamp: function(){
            var
                ...
                calId = that._calId;
            //添加css
            DOM.addClass(trigger, CALENDAR_CLASS);
            //添加id
            DOM.attr(trigger, STAMP_ATTR, calId);
        },
        _loadStyle: function(){
            var
                that = this,
                opts = that._opts;
            if(opts.style && S.isString(opts.style.themeUri)){
                S.loadCSS(opts.style.themeUri);    
            }
        }
    })
...
/**
 * 由于各种原因，暂时不能实际到1.3.0+版本的kissy库
 * 所以日历只能沿用1.2.0的内容，并且对此版本进行本地化操作
 */
 ...
 */
KISSY.add('kscal/base',function (S, Node, Event, undefined) {
 ...
 * @author  拔赤<lijing00333@163.com>
 */
KISSY.add('kscal/page',function (S, Node, Calendar) {
 ...
 * @author  拔赤<lijing00333@163.com>
 */
KISSY.add('kscal/time',function(S, Node,Calendar) {
 ...
 * - 简单的本地化，对w（星期x）的支持
 */
KISSY.add('kscal/date',function (S) {
    ...
    var dateFormat = function () {
        ...
        // Regexes and supporting function_s are cached through closure
 ...
 *  calendar
 */
KISSY.add('kscal/index',function (S, C, Page, Time, Date) {
    C.Date = Date;
    S.Calendar = C;
    return C;
}, {
    requires:["kscal/base", "kscal/page", "kscal/time", "kscal/date"]
});