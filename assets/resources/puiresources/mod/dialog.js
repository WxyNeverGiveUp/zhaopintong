 ...
 */
KISSY.add('mod/dialog', function(S, Base){
    var
        ...
        dataCache = window.top.PW.DATA_CACHE,
        Core = {},
        latestDlgId;
    ...
    function init(){
        ...
        if(!dlgCache){
            window.top.PW.DLG_CACHE = PW.mod.Cache.client();
            dlgCache = window.top.PW.DLG_CACHE;
        }
        if(!dataCache){
            window.top.PW.DATA_CACHE = PW.mod.Cache.client();
            dataCache = window.top.PW.DATA_CACHE;
        } 
    ...
    function buildHotKeyEvt(){
        ...
        hk.bind('esc', function(){
            if(latestDlgId && dlgCache.get(latestDlgId)){
                PW.mod.Dialog.close(latestDlgId);
            }
        })
    }
    ...
    S.augment(Base, S.EventTarget, {
        ...
        _createOverlay: function(){
            var
                ...
                opts = that.opts;
            if(opts.hasOverlay){
                that.overlay = PW.mod.Overlay.client({
                    //默认给window
        ...
        _createDlgEl: function(){
            ...
            HTML += '<div class="cpt-dlg-outer pw-dlg-theme-'+opts.theme+'" theme="theme-'+opts.theme+'" id="dlg-'+that.id+'">';
