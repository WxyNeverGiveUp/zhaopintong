...
* 	初始生成 
* ---------------------------------------------------------------------------*/
KISSY.add('page/employment/rate',function(S,search, exportStu){
	PW.namespace('page.student');
	PW.page.rate = function(param){
		new search(param);
		new exportStu();
	}
},{
	requires:['rate/search','student/export']
});
/*---------------------------------查询------------------------------------*/
KISSY.add('rate/search',function(S){
	var
	...
	S.augment(search,{
		...
		_search:function(){
			...
			StudentIO.searchRate(param, function(code, data, errMsg){
				if(code){
					that._renderRates(data);
				}else{
					Dialog.alert(errMsg);
				}
			});
		...
		_orderBySearch: function(target){
			...
			StudentIO.searchOrderbyRate(info, function(code, data, errMsg){
				if(code){
					that._renderRates(data);
				}else{
					Dialog.alert(errMsg);
