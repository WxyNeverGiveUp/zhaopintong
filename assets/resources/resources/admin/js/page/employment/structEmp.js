/*-----------------------------------------------------------------------------
* @Description: 就业管理--就业结构 (structEmp.js)
* @Version: 	V1.0.0
...
* 	初始生成 
* ---------------------------------------------------------------------------*/
KISSY.add('page/employment/structEmp',function(S,search, exportStu){
	PW.namespace('page.student');
	PW.page.structEmp = function(param){
		new search(param);
		new exportStu();
	}
},{
	requires:['structEmp/search','student/export']
});
/*---------------------------------查询------------------------------------*/
KISSY.add('structEmp/search',function(S){
	var
	...
	S.augment(search,{
		...
		_search:function(){
			...
			StudentIO.searchStructEmp(param, function(code, data, errMsg){
				if(code){
					that._renderStructEmps(data);
				}else{
					Dialog.alert(errMsg);
				}
			});
		...
		_renderStructEmps: function(data){
			var
				that = this;
		...
		_orderBySearch: function(target){
			...
			StudentIO.searchOrderbyStructEmp(info, function(code, data, errMsg){
				if(code){
					that._renderStructEmps(data);
				}else{
					Dialog.alert(errMsg);
