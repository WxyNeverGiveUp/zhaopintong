/*-----------------------------------------------------------------------------
* @Description: 就业管理--生源结构 (structure.js)
* @Version: 	V1.0.0
...
* 	初始生成 
* ---------------------------------------------------------------------------*/
KISSY.add('page/employment/structure',function(S,search, exportStu){
	PW.namespace('page.student');
	PW.page.structure = function(param){
		new search(param);
		new exportStu();
	}
},{
	requires:['structure/search','student/export']
});
/*---------------------------------查询------------------------------------*/
KISSY.add('structure/search',function(S){
	var
	...
	S.augment(search,{
		...
		_search:function(){
			...
			StudentIO.searchStruct(param, function(code, data, errMsg){
				if(code){
					that._renderStructs(data);
				}else{
					Dialog.alert(errMsg);
				}
			});
		...
		_orderBySearch: function(target){
			...
			StudentIO.searchOrderbyStruct(info, function(code, data, errMsg){
				if(code){
					that._renderStructs(data);
				}else{
					Dialog.alert(errMsg);
