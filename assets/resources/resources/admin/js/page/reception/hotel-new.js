...
* v1.0.0(2014.9.25):
     初始生成
* ---------------------------------------------------------------------------*/
KISSY.add('page/reception/hotel-new',function(S,info){
	PW.namespace('page.hotelNew');
	PW.page.hotelNew = function(param){
		new info(param);
	};
},{
	requires:['hotelNew/info']
});
/*--------------------------------------宾馆信息-------------------------------*/
KISSY.add('hotelNew/info',function(S){
	var
		$ = S.all, on = S.Event.on,
