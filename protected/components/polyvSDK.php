...
class PolyvSDK {
	...
	function __construct() {
		...
		$this->_readtoken 		= "7sUjRVELHS-lRUWU8BaBW-0WGQH6mjwr";
		$this->_writetoken 	= "bIvs0HNKLtykCAf-IjGcbjdqvfqG68ro";
		$this->_privatekey = "zrdTxiuVE1";
		$this->_sign = false;//提交参数是否需要签名
	}
	...
	public function getById($vid) {
		...
		$xml = $this->_processXmlResponse('http://v.polyv.net/uc/services/rest?readtoken='.$this->_readtoken.'&method=getById&vid='.$vid.'&format=xml', $xml);
		if($xml) {
			if($xml->error=='0') 
	...
	public function uploadfile($title,$desc,$tag,$cataid,$file) {
		...
		echo 'cataid='.$cataid.'&JSONRPC='.$JSONRPC.'&writetoken='.$this->_writetoken.$this->_privatekey;
		if (extension_loaded('curl')) {
			$ch = curl_init() or die ( curl_error() );
			$timeout = 360000;
            $post = array(
                ...
                'format'=>'xml',
                'Filedata'=>new CURLFile($file->tempName,'',$file->type)
            );
