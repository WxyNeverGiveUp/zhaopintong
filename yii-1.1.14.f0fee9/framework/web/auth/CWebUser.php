...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	/**
	 * @var boolean whether to automatically renew the identity cookie each time a page1 is requested.
	 ...
	 * If that happens during an ajax call, the complete HTML login page1 is returned as the result of that ajax call. That could be
	 * a problem if the ajax call expects the result to be a json array or a predefined string, as the login page1 is ignored in that case.
	...
	/**
	 * Redirects the user browser to the login page1.
	 * Before the redirection, the current URL (if it's not an AJAX url) will be
	 * kept in {@link returnUrl} so that the user browser may be redirected back
	 * to the current page1 after successful login. Make sure you set {@link loginUrl}
	 ...
	 * across page1 requests during a user session.
	 *
	 * @param string $key variable name
